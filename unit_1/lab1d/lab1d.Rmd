...
- <span class="blue">(1) Use the `dotPlot()` function to create a `dotPlot` of the amount of `sugar` in our `food` data.</span>
...
- <span class="blue">(2) Create a more accurate `dotPlot` by including the `nint` option.</span>
  - Using your `food` data spreadsheet, obtain the minimum value for `sugar` by clicking on the `sugar` header to sort in ascending order.
  - Click on the `sugar` header again to sort in descending order and obtain the maximum value for `sugar`.
  - Set `nint` equal to the maximum value for `sugar` minus the minimum value for `sugar` plus one.
  - Use your _History_ pane to see how we included the option `nint` with the `histogram` function.
...
- <span class="blue">(3) Split the `dotPlot` displaying the distribution of grams of `sugar` in two, by faceting on our observations' `salty_sweet` variable.</span>
...
- We can change the _layout_ of our separated plots by including the `layout` option in our `dotPlot` function. 
    - <span class="blue">(6) Add the following option to the code you used to create the `dotPlot` split by `salty_sweet`.</span>
...
```
- **(7) `View` `food_salty` and write down the number of observations in it.**
...
```
- **(8) What do the values `TRUE` and `FALSE` tell us about how our _rule_ applies to the first six snacks in our data? Which of the first six observations were `Salty`?**
...
- <span class="blue">(9) Use `food_salty` to make a `dotPlot` of the `sodium` in our `Salty` snacks.</span>
...
- <span class="blue">(10) `View` the  `my_sub` data we filtered in the above line of code and verify that it only includes salty snacks that have less than 200 calories.</span>


## Put it all together

- **(11) Use an appropriate `dotPlot` to answer each of the following questions:**
    - **(12) About how much `sugar` does the typical sweet snack have?**
    - **(13) How does the typical amount of `sugar` compare when `healthy_level < 3` and when `healthy_level > 3`?**
...
- Because you are now working with subsets of data, it is important to label our plots and make this distinction.
  ...
  - We can use the `main` option to add a title to our plots.
    - <span class="blue">(14) Add the following option to the code you used to create the `dotPlot` of the `sugar` in `Sweet` snacks.</span>
```{r, eval = FALSE, echo = TRUE }
